...
PID pid_left  (&input_left,  &output_left,  &setpoint_left,  kp, ki, kd);
PID pid_right (&input_right, &output_right, &setpoint_right, kp, ki, kd);
...
void right_PID_controller(int gain) {
  temp_right += gain;
  setpoint_right = scale * temp_right;
